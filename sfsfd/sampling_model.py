...
class SamplingModel:
    ...
    The public methods are:
     ...
     - initialize() : This resets the sampler and produces an initial point for tuning
     - tune(x0) : Tunes the SF-SFD object with COBYLA
     - save_results(method="SF-SFD") : Saves the results as a row in a CSV file
    ...
    def initialize(self):
        """ Initialize/reset the sampler.

        Call this method before training a new sampler, or to reset an
        existing sampler.

        Returns:
            float: x0 for the optimization problem.
    ...
    def tune(self, angle_array):
        """ Train the sampler using COBYLA """
    ...
    def save_results(self, method="SF-SFD"):
        """ Log the full set of results generated by a sampler """

        # Write history data to the output file
        fieldnames=["method","discrepancy","maximin","eigen_value",
                    "cumulative","time_to_sol"]
        with open(self.file_name, "a") as file_instance:
    ...
    def iterative_step(self, angle_array):
        ...
        logging.info(f"  expected discrepancy: {criteria_value_for_the_perturbation}")
